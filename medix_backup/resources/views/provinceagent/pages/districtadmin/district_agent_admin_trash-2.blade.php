...
<?php $nav = 'distict_admin'; ?>
                       ...
                       @if($message = Session::get('success'))
                        ...
                        @endif
                         ...
                         <div class="card-content">
                            <h4 class="card-title">District Agents</h4>
                              <form action="/provinceagent/districagents/deletealldistrictagents"  method="post">
                                @csrf
                                @method('DELETE')
                                <button formaction="provinceagent/districagents/deletealldistrictagents" class="btn btn-danger" type="submit" >Delete All Selected </button>
                                 <button formaction="/provinceagent/districagents/recyclealldistrictagents" class="btn btn-danger" type="submit" onclick="return confirm('Are you sure you want to Restore?')">Restore All Selected </button>
                                <div class="row">
                                ...
                                <table id="member_table" class="table table-striped table-no-bordered table-hover"  cellspacing="0" width="100%" style="width:100%" style="border: 1px solid black; ">
                                    ...
                                    <tbody>
                                     @include('provinceagent.pages.districtadmin.district_agent_trash_pagination_data')
    ...
    <script>
        $(document).ready(function(){
            function  fetch_data(page,sort_type,sort_by,district,query)
            {
                $.ajax({

                    url:"districagents/fetch_trash_data?page="+page+"&sortby="+sort_by+"&sorttype="+sort_type+"&district="+district+"&query="+query,
          ...
          $('.filtercheck').change(function(){
               ...
               var page = $('#hidden_page').val();
                ...
                fetch_data(page,sort_type,column_name,district_id,query);
          ...
          $(document).on('keyup','#search',function(){
           ...
           //sorting function
            $(document).on('click','.sorting',function(){
                ...
                fetch_data(page,reverse_order,column_name,district_id,query);
