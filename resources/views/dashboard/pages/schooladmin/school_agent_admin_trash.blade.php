...
@section('title')
  Home-Manage Department Admins Medix
@endsection
@section('content')
                        ...
                        @endif
                         ...
                         <div class="card-content">
                            <h4 class="card-title">Department Admin</h4>
                              <form action="/deletealldepartmentadmins"  method="post">
                                @csrf
                                @method('DELETE')
                                 <button formaction="/deletealldepartmentadmins" class="btn btn-danger" type="submit" onclick="return confirm('Are you sure you want to Delete?')" >Delete All Selected </button>
                                ...
                                <table id="member_table" class="table table-striped table-no-bordered table-hover"  cellspacing="0" width="100%" style="width:100%" style="border: 1px solid black; ">
                                    ...
                                    <tbody>
                                     @include('dashboard.pages.schooladmin.school_agent_trash_pagination_data')
    ...
    <script>
        $(document).ready(function(){
            function  fetch_data(page,sort_type,sort_by,province,district,zone,division,school,query)
            {
                $.ajax({

                    url:"schoolagents/fetch_trash_data?page="+page+"&sortby="+sort_by+"&sorttype="+sort_type+"&province="+province+"&district="+district+"&zone="+zone+"&division="+division+"&school="+school+"&query="+query,
         ...
         console.log(province_id);
                ...
                fetch_data(page,sort_type,column_name,province_id,district_id,zone_id,division_id,school_id,query);
            ...
            $(document).on('click','.sorting',function(){
              ...
              var district_id= $('#did').val();
               ...
               var division_id= $('#divid').val();
                ...
                fetch_data(page,reverse_order,column_name,province_id,district_id,zone_id,division_id,school_id,query);
