                        ...
                        @endif
                         ...
                         <div class="card-content">
                            <h4 class="card-title">Teachers</h4>
                              ...
                              <form action="recycleallmembers"  method="post">
                                @csrf
                                @method('DELETE')
                                <button formaction="recycleallmembers" class="btn btn-danger" type="submit" >Delete All Selected </button>
                                <div class="row">
                                   <div class="col-md-2">
                                      <select class="form-control filtercheck" name="sub_id" value="{{ old('sub_id') }}" id="subid">
                                        <option value="" disable="true"  selected="true"  >Select School</option>
                                    @foreach($subjects as $subject)
                                        <option value="{{$subject->id}}"   data-school_name="{{$subject->id}}">{{$subject->subname}}</option>
                                     @endforeach
                                    </select>
                                ...
                                <table id="member_table" class="table table-striped table-no-bordered table-hover"  cellspacing="0" width="100%" style="width:100%" style="border: 1px solid black; ">
                                    <thead>
                                        ...
                                        <tr >
                                            ...
                                            <th scope="col">No</th>
                                                ...
                                                >Union ID<span id="reg_id_icon"></span></th>
                                    ...
                                    <tbody>
                                     @include('schoolagent.pages.members.member_pagination_data')
                                      ...
                                      <input type="hidden" name="hidden_zone_id" id="hidden_zone_id" value=""/>
                                       <input type="hidden" name="hidden_division_id" id="hidden_division_id" value=""/>
                                        <input type="hidden" name="hidden_school_id" id="hidden_school_id" value=""/>
                                         <input type="hidden" name="hidden_subject_id" id="hidden_subject_id" value=""/>
        ...
        $(document).ready(function(){
            function  fetch_data(page,sort_type,sort_by,province,district,zone,division,school,sub_id,query)
            {
                $.ajax({
                    ...
                    url:"members/fetch_data?page="+page+"&sortby="+sort_by+"&sorttype="+sort_type+"&province="+province+"&district="+district+"&zone="+zone+"&division="+division+"&sub_id="+sub_id+"&school="+school+"&query="+query,
